...
def check_env(cluster_name: str):
    ...
    if required_containers:
        raise EnvironmentError(f'The required container is not started: '
                               f'{",".join(required_containers)}')


def prepare_package_and_config(clp_config: CLPConfig, clp_home: pathlib.Path,
                               docker_clp_home: pathlib.Path):
