...
import {COMPRESSION_JOBS_TABLE_COLUMN_NAMES} from "./constants.js";
...
class CompressionDbManager {
     ...
     */
    constructor (sqlDbConnPool: Pool, compressionJobsTableName: string) {
     ...
     * @return Job objects with fields with the names in
     * `COMPRESSION_JOBS_TABLE_COLUMN_NAMES`
     */
    async getCompressionJobs (lastUpdateTimestampSeconds: number) {
        const queryString = `
            SELECT
