...
class MongoDBManager(DBManager):
    def __init__(self, logger: Optional[Logger], db_uri: str) -> None:
        ...
        self.__db_collections: Dict[str, Collection] = {
            # TODO: The metric collection name is hardcoded in multiple
    ...
    @staticmethod
    def prepare_job_document(job_id: str, creation_ts: float, detection_ts: float, enqueue_ts: float, status: JobStatus, query: Any):
        job_document = {
            "job_id": job_id,
    ...
    def update_job_status(self, job_id: str, status: JobStatus):
        ...
        update_operation = {"$set": {"status": status.to_str()}}
        self.__db_collections["search_jobs_metrics"].find_one_and_update(filter_criteria, update_operation)

    def get_results_count(self, collection_name: str) -> Dict[str, Any]:
        return self.__archive_db[collection_name].count_documents({})
