...
def main(argv):
    ...
    args_parser.add_argument('--context', help="Time context to search") #eg., Last-15-MINUTE, Last-1-HOUR
...
def parsecontext(contextArg):
    contextSplit = contextArg.split("-")
    if len(contextSplit) != 3:
        return -1
    if contextSplit[0] != "Last":
        return -1
    if contextSplit[2] == "HOUR":
        unit = "HOUR"
    elif contextSplit[2] == "MINUTE":
        unit = "MINUTE"
    else:
        return -1
    if not contextSplit[1].isdigit():
        return -1
    context = {'unit': unit, 'interval': int(contextSplit[1])}
    print(context)
    return context
