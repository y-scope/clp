...
def main(argv):
    ...
    try:
        ...
        admin_username = clp_config.database.users[ClpDbUserType.ADMIN].username
        admin_password = clp_config.database.users[ClpDbUserType.ADMIN].password
        regular_username = clp_config.database.users[ClpDbUserType.CLP].username
        ...
        escaped_admin_username = privileged_username.replace("'", "''")
        escaped_admin_password = privileged_password.replace("'", "''")
        ...
        with closing(sql_adapter.create_connection(True)) as db_conn, closing(
            ...
            logger.info("Admin database user '%s' created and permissions applied successfully.", admin_username)
