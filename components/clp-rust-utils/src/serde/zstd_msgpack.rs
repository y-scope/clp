...
mod tests {
    ...
    fn serialize_produces_decompressible_named_messagepack() -> Result<(), Box<dyn StdError>> {
        ...
        let compressed = ZstdMsgpack::serialize(&expected)?;
        let deserialized: TestValue = ZstdMsgpack::deserialize(&compressed)?;
    ...
    #[test]
    fn deserialize_rejects_invalid_frame() {
        let error = ZstdMsgpack::deserialize::<TestValue>(b"not a zstd frame")
            .expect_err("invalid zstd data should fail");
        let Error::Zstd(message) = error else {
            panic!("expected a zstd error");
        };
        assert_eq!("invalid zstd frame", message);
    }
