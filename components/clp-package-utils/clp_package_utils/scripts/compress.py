...
def main(argv):
    ...
    if instance_id_file_path.exists():
        ...
        logger.error("No instance id found. Check if package has been launched")
        return -1

    container_name = f'clp-compression-issuer-{instance_id}'
    ...
    for path in parsed_args.paths:
        ...
        # Note, here we don't prepend with CONTAINER_INPUT_LOGS_ROOT_DIR
        # the simple assumption is that the compress won't process the file
        compress_cmd.append(str(resolved_path))
            ...
            with open(output_list_path, 'w') as output_list:
                ...
                with open(parsed_args.input_list, 'r') as f:
                    for line in f:
                        ...
                        output_list.write(f'{resolved_path}\n')
