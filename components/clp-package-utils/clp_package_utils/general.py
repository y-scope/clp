...
def validate_and_load_compression_queue_credentials_file(clp_config: CLPConfig, clp_home: pathlib.Path,
                                                         generate_default_file: bool):
    validate_credentials_file_path(clp_config, clp_home, generate_default_file)
    clp_config.load_compression_queue_credentials_from_file()
...
def validate_queue_config(component_name: str, queue_config: Queue, logs_dir: pathlib.Path):
    try:
        validate_path_could_be_dir(logs_dir)
    except ValueError as ex:
        raise ValueError(f"{component_name} logs directory is invalid: {ex}")

    validate_port(f"{component_name}.port", queue_config.host, queue_config.port)


def validate_results_cache_config(clp_config: CLPConfig, data_dir: pathlib.Path,
...
def validate_webui_query_handler_config(clp_config: CLPConfig):
    validate_port(f"{WEBUI_QUERY_HANDLER_COMPONENT_NAME}.port", clp_config.webui_query_handler.host,
                  clp_config.webui_query_handler.port)

    if clp_config.webui_query_handler.max_results < 1:
        raise ValueError(f"{WEBUI_QUERY_HANDLER_COMPONENT_NAME}.max-results cannot be less than 1")
