...
if [[ "$telemetry_prompt_needed" == "true" ]]; then
    if [[ -t 0 ]]; then
        ...
        echo "This includes: CLP version, OS/architecture, deployment method,"
        echo "performance metrics, and component health status. It does NOT include:"
        echo "log content, queries, hostnames, IP addresses, or any personally"
        echo "identifiable information."
        echo ""
