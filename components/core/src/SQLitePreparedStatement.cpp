...
SQLitePreparedStatement::~SQLitePreparedStatement () {
    // NOTE: sqlite3_finalize can return an error but the docs seem to imply this is not a failure of finalize but rather a notification that the statement was
    //  not in a good state before finalization.
    sqlite3_finalize(m_statement_handle);
    m_statement_handle = nullptr;
    m_db_handle = nullptr;
}

SQLitePreparedStatement::SQLitePreparedStatement (SQLitePreparedStatement&& rhs) noexcept : m_db_handle(nullptr), m_statement_handle(nullptr),
        m_row_ready(false)
{
    *this = std::move(rhs);
}

SQLitePreparedStatement& SQLitePreparedStatement::operator= (SQLitePreparedStatement&& rhs) noexcept {
    if (this != &rhs) {
...
void SQLitePreparedStatement::bind_text (int parameter_index, const std::string& value, bool copy_parameter) {
    auto return_value = sqlite3_bind_text(m_statement_handle, parameter_index, value.c_str(), value.length(),
...
void SQLitePreparedStatement::reset () {
    // NOTE: sqlite3_reset can return an error but the docs seem to imply this is not a failure of reset but rather a notification that the statement was
    //  not in a good state before reset.
