...
struct fmt::formatter<ErrorCode> {
    ...
    auto format(ErrorCode const& error_code, FormatContext& ctx) {
...
struct fmt::formatter<ffi::search::ExactVariableToken<encoded_variable_t>> {
    ...
    template <typename FormatContext>
    auto format(ffi::search::ExactVariableToken<encoded_variable_t> const& v, FormatContext& ctx) {
...
struct fmt::formatter<ffi::search::WildcardToken<encoded_variable_t>> {
    ...
    template <typename FormatContext>
    auto format(ffi::search::WildcardToken<encoded_variable_t> const& v, FormatContext& ctx) {
        return fmt::format_to(
                ctx.out(),
