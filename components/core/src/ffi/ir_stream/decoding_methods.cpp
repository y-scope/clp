...
namespace ffi::ir_stream {
     ...
     * to decode
     */
    static IRErrorCode parse_logtype (ReaderInterface& reader, encoded_tag_t encoded_tag,
                                      string& logtype);
     ...
     */
    template <typename encoded_variable_t>
    IRErrorCode parse_timestamp (ReaderInterface& reader, encoded_tag_t encoded_tag,
                                 epoch_time_ms_t& ts);
     ...
     */
    template <typename encoded_variable_t>
    static IRErrorCode generic_decode_next_message (ReaderInterface& reader,
                                                    string& message,
    ...
    template <typename encoded_variable_t>
    IRErrorCode parse_timestamp (ReaderInterface& reader, encoded_tag_t encoded_tag, epoch_time_ms_t& ts)
    {
        static_assert(is_same_v<encoded_variable_t, eight_byte_encoded_variable_t> ||
                      is_same_v<encoded_variable_t, four_byte_encoded_variable_t>);
                                                    ...
                                                    epoch_time_ms_t& timestamp)
    {
        ...
        vector<string> dict_vars;
        string logtype;
        if (auto error_code = generic_parse_tokens(reader, logtype, encoded_vars, dict_vars,
                                                   timestamp);
            IRErrorCode_Success != error_code) {
            return error_code;
        }
        ...
        // constant handler
        auto constant_handler = [&message] (const string& value, size_t begin_pos, size_t length) {
        ...
        // dict var handler
        auto dict_var_handler = [&message] (const string& dict_var) {
            message.append(dict_var);
        };

        try {
            generic_decode_message(logtype, encoded_vars, dict_vars,
    ...
    static IRErrorCode read_metadata_info (ReaderInterface& reader, encoded_tag_t& metadata_type,
    ...
    IRErrorCode decode_preamble (ReaderInterface& reader, encoded_tag_t& metadata_type,
                                 ...
                                 std::vector<int8_t>& metadata)
    {
        ...
        if (ErrorCode_Success != reader.try_read_exact_length(
                size_checked_pointer_cast<char>(metadata.data()), metadata_size)) {
            return IRErrorCode_Incomplete_IR;
        }
        return IRErrorCode_Success;
    }
    ...
    namespace four_byte_encoding {
        IRErrorCode decode_next_message (ReaderInterface& reader, string& message,
