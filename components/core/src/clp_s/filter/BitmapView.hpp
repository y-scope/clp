 ...
 * @param bit_handler
 * @param bit_ix The index of the set bit in the bitmap.
 * @return A boolean result indicating whether the bit should remain set, or an error code
 * indicating the failure.
 */
template <typename SetBitVisitor>
concept SetBitVisitorConcept = requires(SetBitVisitor bit_handler, size_t bit_ix) {
    { bit_handler(bit_ix) } -> std::same_as<ystdlib::error_handling::Result<bool>>;
};
...
public:
    ...
    [[nodiscard]] auto set_bit(size_t index, bool value) -> ystdlib::error_handling::Result<void> {
        ...
        auto const bit_mask{static_cast<bitmap_component_t>(1) << bit_offset};
        if (value) {
            m_bitmap_array[component_ix] |= bit_mask;
        } else {
            m_bitmap_array[component_ix] &= ~bit_mask;
        }
    ...
    [[nodiscard]] auto filter_set_bits(SetBitVisitor set_bit_visitor)
        ...
        for (size_t component_ix{}; component_ix < m_bitmap_array_length; ++component_ix) {
            if (0 == m_bitmap_array[component_ix]) {
                continue;
            }

            auto const current_component{m_bitmap_array[component_ix]};
            bitmap_component_t new_component{};
            auto const last_component_ix{m_bitmap_array_length - 1};
            auto const max_bit{
                    component_ix == last_component_ix
                            ? m_num_bits - cNumBitsPerComponent * last_component_ix
                            : cNumBitsPerComponent
            };
            for (size_t bit_ix{}; bit_ix < max_bit; ++bit_ix) {
                auto const current_bit{static_cast<bitmap_component_t>(1) << bit_ix};
                if (0 != (current_component & current_bit)) {
                    auto const current_ix{bit_ix + cNumBitsPerComponent * component_ix};
                    if (YSTDLIB_ERROR_HANDLING_TRYX(set_bit_visitor(current_ix))) {
                        new_component |= current_bit;
                    }
                }
            }
            m_bitmap_array[component_ix] = new_component;
        }
        return ystdlib::error_handling::success();
    }
