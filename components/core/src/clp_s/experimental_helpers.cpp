...
auto get_capture_positions(
    ...
    auto const register_ids{lexer.get_reg_ids_from_capture_id(id)};
...
auto is_leaf_capture(
    ...
    if (next_idx == capture_ids.size()) {
        return true;
    }

    auto const& next_capture_id{capture_ids[next_idx]};
    auto [unused, next_end_pos]{YSTDLIB_ERROR_HANDLING_TRYX(
            get_capture_positions(event, root_variable, next_capture_id)
    )};
...
auto sort_capture_ids(
    ...
    std::sort(capture_ids.begin(), capture_ids.end(), [&](auto& a, auto& b) -> bool {
        static constexpr std::string_view cErrorFmt{
                "Could not get positions for capture id: {}, error: {} ({})"
        };
