...
void TimestampEntry::ingest_timestamp(epochtime_t timestamp) {
    ...
    if (millisecond_timestamp_lower_bound < m_epoch_start) {
        m_epoch_start = millisecond_timestamp_lower_bound;
    }
    if (millisecond_timestamp_upper_bound > m_epoch_end) {
        m_epoch_end = millisecond_timestamp_upper_bound;
    }
}
