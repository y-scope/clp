...
public:
     ...
     */
    auto insert_ordered(int32_t mst_node_id) -> void;

    /**
     * Inserts a node into the unordered region of the schema.
     */
    auto insert_unordered(int32_t mst_node_id) -> void;
     ...
     */
    auto operator[](size_t i) const -> int32_t { return m_schema[i]; }

    /**
     * @return a view into the ordered region of the underlying schema
     */
    [[nodiscard]] auto get_ordered_schema_view() -> std::span<int32_t> {
     ...
     */
    [[nodiscard]] auto get_view(size_t i, size_t size) -> std::span<int32_t> {
        if (i > m_schema.size() || size > m_schema.size() - i) {
            throw OperationFailed(ErrorCodeOutOfBounds, __FILENAME__, __LINE__);
        }
        return std::span<int32_t>{m_schema}.subspan(i, size);
    }
     ...
     */
    static auto encode_node_type_as_schema_entry(NodeType node_type) -> int32_t {
     ...
     */
    static auto schema_entry_is_unordered_object(int32_t schema_entry) -> bool {
