 ...
 */
static GlobalMetadataDB::ArchiveIterator*
get_archive_iterator(GlobalMetadataDB& global_metadata_db, std::string const& file_path);
...
int main(int argc, char const* argv[]) {
    ...
    try {
        ...
        /// TODO: if performance is too slow, can make this more efficient by only diffing files
        ...
        std::vector<std::string> const archive_paths = command_line_args.get_archive_path();
        ...
        // ==================
        for (std::string const& archive_path : archive_paths) {
            ...
            if (std::filesystem::exists(schema_file_path)) {
                ...
                if (num_bytes_read < max_map_schema_length) {
                    ...
                    // if there is a chance there might be a difference make a new lexer as it's
                    // pretty fast to create
                    if (forward_lexer_map_it == forward_lexer_map.end()) {
                        // Create forward lexer
                        auto insert_result = forward_lexer_map.emplace(
                                buf,
                                compressor_frontend::lexers::ByteLexer()
                        );
                        ...
                        insert_result = reverse_lexer_map.emplace(
                                buf,
                                compressor_frontend::lexers::ByteLexer()
                        );
