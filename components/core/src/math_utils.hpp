 ...
 * @return The given value rounded up to the nearest multiple of the given
 * factor
 */
...
auto int_round_up_to_multiple(unsigned_t val, unsigned_t factor) -> unsigned_t {
    ...
    // NOTE: "val + multiple" could overflow, but the "- 1" will undo the
    // overflow since overflow semantics are well-defined for unsigned integers.
