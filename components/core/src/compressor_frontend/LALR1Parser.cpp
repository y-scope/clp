...
namespace compressor_frontend {
    ...
    void LALR1Parser::add_token_chain (const string& name, const string& chain) {
        ...
        unique_ptr<RegexASTCat> rule_chain = make_unique<RegexASTCat>(std::move(first_char_rule), std::move(second_char_rule));
        for (uint32_t i = 2; i < chain.size(); i++) {
            ...
            rule_chain = make_unique<RegexASTCat>(std::move(rule_chain), std::move(next_char_rule));
        }
    ...
    LALR1Parser::LALR1Parser () : m_archive_writer_ptr(nullptr), root_itemset_ptr(nullptr), m_root_production_id(0) {
    ...
    string LALR1Parser::report_error (ReaderInterface& reader) {
        ...
        if (token.m_type_ids->at(0) == (int) SymbolID::TokenEndID && consumed_input.empty()) {
            ...
            for (uint32_t i = 0; i < m_parse_stack_states.top()->m_actions.size(); i++) {
                ...
                if (action.index() != 0) {
                    /*
                    if (it.first == m_lexer.m_symbol_id["Space"]) {
                        continue;
                    }
                    */
                    error_type += "'";
