...
namespace compressor_frontend {
    MatchedSymbol NonTerminal::all_children[cSizeOfAllChildren];
    ...
    void LALR1Parser::add_rule (const string& name, unique_ptr<RegexAST> rule) {
        ...
        m_lexer.add_rule(m_lexer.symbol_id[name], move(rule));
        m_terminals.insert(m_lexer.symbol_id[name]);
    }

    void LALR1Parser::add_token (const string& name, char rule_char) {
        add_rule(name, make_unique<RegexASTLiteral>(RegexASTLiteral(rule_char)));
    }

    void LALR1Parser::add_token_group (const string& name, unique_ptr<RegexASTGroup> rule_group) {
        add_rule(name, move(rule_group));
    }

    void LALR1Parser::add_token_chain (const string& name, const string& chain) {
        assert(chain.size() > 1);
        unique_ptr<RegexASTLiteral> first_char_rule = make_unique<RegexASTLiteral>(RegexASTLiteral(chain[0]));
        unique_ptr<RegexASTLiteral> second_char_rule = make_unique<RegexASTLiteral>(RegexASTLiteral(chain[1]));
        unique_ptr<RegexASTCat> rule_chain = make_unique<RegexASTCat>(move(first_char_rule), move(second_char_rule));
        for (uint32_t i = 2; i < chain.size(); i++) {
            char next_char = chain[i];
            unique_ptr<RegexASTLiteral> next_char_rule = make_unique<RegexASTLiteral>(RegexASTLiteral(next_char));
            rule_chain = make_unique<RegexASTCat>(move(rule_chain), move(next_char_rule));
        }
        add_rule(name, move(rule_chain));
    }

    uint32_t LALR1Parser::add_production (const string& head, const vector<string>& body,
        ...
        p->semantic_rule = move(semantic_rule);
        m_nonterminals.insert(pair<int, vector<Production*>>(p->head, {}));
        m_nonterminals[p->head].push_back(p.get());
        m_productions_map[head][body] = p.get();
        m_productions.push_back(move(p));
    ...
    LALR1Parser::LALR1Parser () : archive_writer_ptr(nullptr), root_itemset_ptr(nullptr), root_production_id(0) {
        m_lexer.symbol_id[cTokenEnd] = (int) SymbolID::TokenEndID;
        m_lexer.symbol_id[cTokenUncaughtString] = (int) SymbolID::TokenUncaughtStringID;
        m_lexer.symbol_id[cTokenInt] = (int) SymbolID::TokenIntId;
        m_lexer.symbol_id[cTokenDouble] = (int) SymbolID::TokenDoubleId;
        m_lexer.symbol_id[cTokenFirstTimestamp] = (int) SymbolID::TokenFirstTimestampId;
        m_lexer.symbol_id[cTokenNewlineTimestamp] = (int) SymbolID::TokenNewlineTimestampId;
        m_lexer.symbol_id[cTokenNewline] = (int) SymbolID::TokenNewlineId;

        m_lexer.id_symbol[(int) SymbolID::TokenEndID] = cTokenEnd;
        m_lexer.id_symbol[(int) SymbolID::TokenUncaughtStringID] = cTokenUncaughtString;
        m_lexer.id_symbol[(int) SymbolID::TokenIntId] = cTokenInt;
        m_lexer.id_symbol[(int) SymbolID::TokenDoubleId] = cTokenDouble;
        m_lexer.id_symbol[(int) SymbolID::TokenFirstTimestampId] = cTokenFirstTimestamp;
        m_lexer.id_symbol[(int) SymbolID::TokenNewlineTimestampId] = cTokenNewlineTimestamp;
        m_lexer.id_symbol[(int) SymbolID::TokenNewlineId] = cTokenNewline;
    ...
    void LALR1Parser::generate_lr0_kernels () {
        ...
        m_lr0_itemsets[item_set0->kernel] = move(item_set0);
    ...
    bool LALR1Parser::lr_closure_helper (ItemSet* item_set_ptr, const Item* item, uint32_t* next_symbol) {
    ...
    void LALR1Parser::generate_lr0_closure (ItemSet* item_set_ptr) {
        deque<Item> q(item_set_ptr->kernel.begin(), item_set_ptr->kernel.end()); // {{S'->(dot)S, ""}}
        while (!q.empty()) {
    ...
    ItemSet* LALR1Parser::go_to (ItemSet* from_item_set, const uint32_t& next_symbol) {
        ...
        if (m_lr0_itemsets.find(next_item_set_ptr->kernel) != m_lr0_itemsets.end()) {
            ...
            m_lr0_itemsets[next_item_set_ptr->kernel] = move(next_item_set_ptr);
    ...
    void LALR1Parser::generate_first_sets () {
        ...
        while (changed) {
            ...
            for (const unique_ptr<Production>& p: m_productions) {
                set<uint32_t>& f = m_firsts[p->head];
                if (p->is_epsilon()) {
    ...
    void LALR1Parser::generate_lr1_itemsets () {
        ...
        for (map<set<Item>, unique_ptr<ItemSet>>::value_type const& kv: m_lr0_itemsets) {
            ...
            m_lr1_itemsets[lr1_item_set_ptr->kernel] = move(lr1_item_set_ptr);
        }
        // this seems like the wrong way to do this still:
        for (map<set<Item>, unique_ptr<ItemSet>>::value_type const& kv1: m_lr1_itemsets) {
            for (map<int, int>::value_type next_index: go_to_table[kv1.second->index]) {
                bool success = false;
    ...
    void LALR1Parser::generate_lr1_closure (ItemSet* item_set_ptr) {
        deque<Item> queue(item_set_ptr->kernel.begin(), item_set_ptr->kernel.end());
        while (!queue.empty()) {
...
// Dragon book page 253
            ...
            for (Item const& item: item_set_ptr->closure) {
                if (!item.has_dot_at_end()) {
                    ...
                    if (!holds_alternative<bool>(action)) {
                        if (holds_alternative<ItemSet*>(action) && std::get<ItemSet*>(action) == item_set_ptr->next[item.next_symbol()]) {
                            continue;
                        }
                        cout << "Warning: For symbol " << m_lexer.id_symbol[item.next_symbol()] << ", adding shift to "
                                  << item_set_ptr->next[item.next_symbol()]->index << " causes ";
                        if (holds_alternative<ItemSet*>(action)) {
                            cout << "shift-shift conflict with shift to " << std::get<ItemSet*>(action)->index << std::endl;
                        } else {
                            cout << "shift-reduce conflict with reduction " << m_lexer.id_symbol[std::get<Production*>(action)->head]
                                      << "-> {";
                            for (uint32_t symbol: std::get<Production*>(action)->body) {
                                cout << m_lexer.id_symbol[symbol] << ",";
                            }
                            cout << "}" << std::endl;
                        }
                    }
                ...
                if (item.has_dot_at_end()) {
                    if (item.production == m_productions[root_production_id].get()) {
                        Action action = true;
                        item_set_ptr->actions[(int) SymbolID::TokenEndID] = action;
                    } else {
                        Action& action = item_set_ptr->actions[item.lookahead];
                        if (!holds_alternative<bool>(action)) {
                            cout << "Warning: For symbol " << m_lexer.id_symbol[item.lookahead]
                            ...
                            cout << "} causes ";
                            if (holds_alternative<ItemSet*>(action)) {
                                cout << "shift-reduce conflict with shift to " << std::get<ItemSet*>(action)->index << std::endl;
                            } else {
            ...
            std::visit(overloaded{
                    ...
                    [&symbols] (NonTerminal& m) {
                        for (int i = 0; i < m.production->body.size(); i++) {
                            symbols.push(move(NonTerminal::all_children[m.children_start + i]));
                        }
                    }
            }, curr_symbol);
            symbols.pop();
    ...
    string LALR1Parser::get_input_after_last_newline (std::stack<MatchedSymbol>& parse_stack_matches) {
        string error_message_reversed;
        bool done = false;
        while (!parse_stack_matches.empty() && !done) {
            MatchedSymbol top_symbol = move(parse_stack_matches.top());
            ...
            std::visit(overloaded{
                    ...
                    [&parse_stack_matches] (NonTerminal& m) {
                        for (int i = 0; i < m.production->body.size(); i++) {
                            parse_stack_matches.push(move(NonTerminal::all_children[m.children_start + i]));
                        }
                    }
            }, top_symbol);
        }
    ...
    string LALR1Parser::get_input_until_next_newline (ReaderInterface& reader, Token* error_token) {
        string rest_of_line;
        bool next_is_end_token = (error_token->type_ids->at(0) == (int) SymbolID::TokenEndID);
        ...
        while (!next_has_newline && !next_is_end_token) {
            ...
            if (!next_has_newline) {
                ...
                next_is_end_token = (token.type_ids->at(0) == (int) SymbolID::TokenEndID);
            }
        }
        rest_of_line += "\n";
        return rest_of_line;
    ...
    string LALR1Parser::report_error (ReaderInterface& reader) {
        assert(m_next_token == std::nullopt);
        assert(!m_parse_stack_matches.empty());
        MatchedSymbol top_symbol = move(m_parse_stack_matches.top());
        ...
        error_indicator += "^\n";
        if (token.type_ids->at(0) == (int) SymbolID::TokenEndID && consumed_input.empty()) {
    ...
    NonTerminal LALR1Parser::parse (ReaderInterface& reader) {
        ...
        assert(!m_parse_stack_matches.empty());
        MatchedSymbol m = move(m_parse_stack_matches.top());
        m_parse_stack_matches.pop();
        assert(m_parse_stack_matches.empty());
        return move(std::get<NonTerminal>(m));
    }
    ...
    Token LALR1Parser::get_next_symbol () {
        ...
        Token s = move(m_next_token.value());
        m_next_token = std::nullopt;
        return s;
    }
    ...
    bool LALR1Parser::parse_advance (Token& next_token, bool* accept) {
        ...
        assert(*accept == false);
        // For error handling
        m_parse_stack_matches.push(move(next_token));
        return true;
    }
    ...
    bool LALR1Parser::parse_symbol (uint32_t const& type_id, Token& next_token, bool* accept) {
        ...
        std::visit(overloaded{
                ...
                [&ret, &next_token, this] (Production* reduce) {
                    m_next_token = move(next_token);
                    NonTerminal matched_nonterminal(reduce);
                    size_t n = reduce->body.size();
                    for (size_t i = 0; i < n; i++) {
                        m_parse_stack_states.pop();
                        NonTerminal::all_children[matched_nonterminal.children_start + n - i - 1] = move(m_parse_stack_matches.top());
                        m_parse_stack_matches.pop();// matched_nonterminal.children->at(n - i - 1) = move(m);
                    }
                    if (reduce->semantic_rule != nullptr) {
                        m_lexer.set_reduce_pos(m_next_token->start_pos - 1);
                    ...
                    }
                    {
                        ...
                        m_parse_stack_matches.push(move(matched_nonterminal));
                    }
                    ret = true;
                    return;
                }
