...
auto LogEventDeserializer<encoded_variable_t>::create(ReaderInterface& reader)
    ...
    if (metadata_json.is_discarded()) {
        return ffi::ir_stream::IrErrorCode{ffi::ir_stream::IrErrorCodeEnum::DecodingMethodFailure};
    }
    ...
    if constexpr (std::is_same_v<encoded_variable_t, four_byte_encoded_variable_t>) {
        ...
        if (metadata_json.end() == ref_timestamp_iter || false == ref_timestamp_iter->is_string()) {
            return ffi::ir_stream::IrErrorCode{
                    ffi::ir_stream::IrErrorCodeEnum::DecodingMethodFailure
            };
        }
...
auto LogEventDeserializer<encoded_variable_t>::deserialize_log_event() -> ystdlib::error_handling::
    ...
    auto const ir_error_code = ffi::ir_stream::deserialize_log_event(
            m_reader,
            tag,
            logtype,
            encoded_vars,
