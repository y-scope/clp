...
public:
    ...
    LatestResultsChecker(
            uint64_t num_latest_results,
            std::set<segment_id_t> const& segments_to_search
    )
            : m_num_latest_results(num_latest_results),
    ...
    bool need_to_scan(clp::streaming_archive::reader::File& compressed_file) {
        if (m_num_latest_results == 0) {
            return true;
        }

        epochtime_t begin_ts = compressed_file.get_begin_ts();
        epochtime_t end_ts = compressed_file.get_end_ts();
        if (m_num_accumulated_results >= m_num_latest_results && end_ts < last_begin_ts) {
            return true;
        }

        if (m_num_accumulated_results < m_num_latest_results) {
...
private:
    uint64_t m_num_latest_results, m_num_accumulated_results;
