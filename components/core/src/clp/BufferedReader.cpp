...
BufferedReader::BufferedReader(
    ...
    m_in_src_pos = m_reader->get_pos();
    m_buffer_begin_in_src_pos = 0;
    m_buffer_reader = BufferReader{m_buffer.data(), 0};
    m_highest_read_in_src_pos = m_in_src_pos;
}

auto BufferedReader::try_refill_buffer_if_empty() -> ErrorCode {
...
auto BufferedReader::peek_buffered_data(char const*& buf, size_t& peek_size) const -> void {
    m_buffer_reader.peek_buffer(buf, peek_size);
}

auto BufferedReader::set_checkpoint() -> size_t {
    if (m_checkpoint_pos.has_value() && m_checkpoint_pos.value() < m_in_src_pos
        && m_buffer_reader.get_buffer_size() != m_base_buffer_size)
    {
        drop_content_before_current_pos();
    }
    m_checkpoint_pos = m_in_src_pos;
    return m_in_src_pos;
}
...
auto BufferedReader::try_get_pos(size_t& pos) -> ErrorCode {
    pos = m_in_src_pos;
    return ErrorCode_Success;
}

auto BufferedReader::try_seek_from_begin(size_t pos) -> ErrorCode {
    if (pos == m_in_src_pos) {
        return ErrorCode_Success;
    }

    auto seek_lower_bound = m_checkpoint_pos.has_value() ? m_checkpoint_pos.value() : m_in_src_pos;
    if (pos < seek_lower_bound) {
        return ErrorCode_Unsupported;
    }

    auto error_code = m_buffer_reader.try_seek_from_begin(get_in_buffer_pos(pos));
    if (ErrorCode_Truncated == error_code) {
        if (false == m_checkpoint_pos.has_value()) {
            // If checkpoint is not set, simply invalidate the buffer reader
            ...
            m_buffer_reader = BufferReader{m_buffer.data(), 0};
            m_buffer_begin_in_src_pos = pos;
        } else {
            auto const num_bytes_to_refill = pos - get_buffer_end_in_src_pos();
            ...
            error_code = m_buffer_reader.try_seek_from_begin(get_in_buffer_pos(pos));
            if (ErrorCode_Success != error_code) {
                return error_code;
            }
        }
...
auto BufferedReader::try_read(char* buf, size_t num_bytes_to_read, size_t& num_bytes_read)
    ...
    while (false == dst_view.empty()) {
        ...
        if (ErrorCode_Success == error_code) {
            ...
            update_in_src_pos(m_in_src_pos + bytes_read);
...
BufferedReader::try_read_to_delimiter(char delim, bool keep_delimiter, bool append, string& str)
    ...
    while (true) {
        ...
        update_in_src_pos(m_in_src_pos + num_bytes_read);
...
auto BufferedReader::refill_reader_buffer(size_t num_bytes_to_refill) -> ErrorCode {
    auto const buffer_end_in_src_pos = get_buffer_end_in_src_pos();
    ...
    if (m_checkpoint_pos.has_value()) {
        ...
        num_bytes_to_read = m_base_buffer_size - (buffer_end_in_src_pos % m_base_buffer_size);
        if (num_bytes_to_read > available_buffer_space) {
            // Advance the entire buffer since we don't grow the buffer if there's no checkpoint
            next_buffer_pos = 0;
            next_buffer_begin_pos = buffer_end_in_src_pos;
        } else {
            next_buffer_pos = data_size;
        }
    }
    ...
    m_buffer_begin_in_src_pos = next_buffer_begin_pos;
    return error_code;
}

auto BufferedReader::drop_content_before_current_pos() -> void {
    ...
    m_buffer_begin_in_src_pos += buffer_reader_pos;

    m_buffer_reader = BufferReader{m_buffer.data(), new_data_size};
}

auto BufferedReader::update_in_src_pos(size_t pos) -> void {
    m_in_src_pos = pos;
    m_highest_read_in_src_pos = std::max(m_in_src_pos, m_highest_read_in_src_pos);
}
}  // namespace clp
