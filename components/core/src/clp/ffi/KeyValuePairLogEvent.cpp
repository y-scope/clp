 ...
 * @param kv_pairs
 * @param schema_tree
 * @return A result containing the bitmap where the node IDs appeared in the schema of `kv_pairs`
 * are set, or an error code indicating the failure:
...
/**
 * Decodes a value as an `EncodedTextAst`.
 * @param val
...
auto get_sub_schema_tree_bitmap(
    ...
    for (auto const& [key_id, val] : kv_pairs) {
        if (key_id >= sub_schema_tree_bitmap.size()) {
            return std::errc::result_out_of_range;
        }
        sub_schema_tree_bitmap[key_id] = true;

        // Iteratively mark the parents as true
        auto parent_id{schema_tree.get_node(key_id).get_parent_id()};
        while (true) {
...
auto KeyValuePairLogEvent::serialize_to_json(
    ...
    auto const sub_schema_tree_bitmap_ret{get_sub_schema_tree_bitmap(m_kv_pairs, *m_schema_tree)};
    ...
    while (false == dfs_stack.empty() && false == json_exception_captured) {
        ...
        if (m_kv_pairs.contains(child_id)) {
            // Dealing with leaf nodes
            if (false
                == insert_kv_pair_json_map(child_node, m_kv_pairs.at(child_id), top.get_map()))
            {
                return std::errc::protocol_error;
            }
        } else {
