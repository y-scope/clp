...
void open_dictionary_for_reading(
        ...
        unique_ptr<FileReader>& dictionary_file_reader,
        streaming_compression::Decompressor& dictionary_decompressor,
        unique_ptr<FileReader>& segment_index_file_reader,
        streaming_compression::Decompressor& segment_index_decompressor
) {
    dictionary_file_reader = move(make_unique<FileReader>(dictionary_path));
    // Skip header
    ...
    segment_index_file_reader = move(make_unique<FileReader>(segment_index_path));
    // Skip header
