...
MySQLDB::Iterator::Iterator (Iterator&& rhs) noexcept : m_query_result(nullptr), m_row(nullptr), m_field_lengths(nullptr), m_num_fields(0) {
    *this = std::move(rhs);
}
...
bool MySQLDB::execute_query (const string& sql_query) {
    ...
    if (0 != mysql_real_query(m_db_handle, sql_query.c_str(), sql_query.length())) {
        SPDLOG_ERROR("MySQLDB: Query failed - {}. ({})", mysql_error(m_db_handle), sql_query.c_str());
        return false;
    }
