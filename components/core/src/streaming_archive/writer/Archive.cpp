...
void Archive::open(UserConfig const& user_config) {
    ...
    try {
        ...
        // Update size before we write the metadata file so we can store the size in the metadata
        // file
        m_stable_size += sizeof(cArchiveFormatVersion) + sizeof(m_stable_uncompressed_size)
                         + sizeof(m_stable_size);
    ...
    m_global_metadata_db->add_archive(
            m_id_as_string,
            m_stable_uncompressed_size,
            m_stable_size,
...
void Archive::write_msg_using_schema(
        compressor_frontend::Token*& uncompressed_msg,
        uint32_t uncompressed_msg_pos,
        bool const has_delimiter,
...
void Archive::update_metadata() {
    ...
    m_metadata_file_writer.seek_from_current(-((int64_t)(sizeof(m_stable_uncompressed_size)
                                                         + sizeof(m_stable_size))));
    ...
    m_global_metadata_db
            ->update_archive_size(m_id_as_string, stable_uncompressed_size, stable_size);
