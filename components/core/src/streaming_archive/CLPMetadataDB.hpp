...
namespace streaming_archive::clp {
    ...
    public:
        ...
        public:
            ...
            public:
                // Constructors
                OperationFailed (ErrorCode error_code, const char* const filename,
                                 int line_number)
            ...
            // Constructors
            explicit CLPFileIterator (MetadataDB* m_db_ptr, SQLiteDB& db,
                                      epochtime_t begin_timestamp, epochtime_t end_timestamp,
                                      const std::string& file_path, bool in_specific_segment,
                                      segment_id_t segment_id);

            // Methods
            [[nodiscard]] size_t get_segment_timestamps_pos() const;
            [[nodiscard]] size_t get_segment_logtypes_pos () const;
            [[nodiscard]] size_t get_segment_variables_pos() const;

        };

        // Methods
        [[nodiscard]] virtual std::unique_ptr<FileIterator> get_file_iterator (
                epochtime_t begin_ts, epochtime_t end_ts, const std::string& file_path,
                bool in_specific_segment, segment_id_t segment_id)
        {
            return std::make_unique<CLPFileIterator>(this, m_db, begin_ts, end_ts, file_path,
    ...
    private:
        ...
        size_t get_field_size() override {
            return enum_to_underlying_type(CLPFilesTableFieldIndexes::Length);
        }
