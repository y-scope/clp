...
MySQLPreparedStatement::MySQLPreparedStatement (MYSQL* db_handle) : m_db_handle(db_handle), m_is_set(false) {
...
MySQLPreparedStatement::MySQLPreparedStatement (MySQLPreparedStatement&& rhs) noexcept : m_db_handle(nullptr), m_statement_handle(nullptr), m_is_set(false) {
    *this = std::move(rhs);
}
...
bool MySQLPreparedStatement::execute () {
    if (0 != mysql_stmt_bind_param(m_statement_handle, m_statement_bindings.get_internal_mysql_bindings())) {
        SPDLOG_ERROR("MySQLPreparedStatement: Failed to bind parameters to statement - {}.", mysql_stmt_error(m_statement_handle));
        return false;
    }

    if (0 != mysql_stmt_execute(m_statement_handle)) {
        SPDLOG_ERROR("MySQLPreparedStatement: Failed to execute statement - {}.", mysql_stmt_error(m_statement_handle));
        return false;
    }
...
void MySQLPreparedStatement::close () {
    if (nullptr != m_statement_handle) {
        if (0 != mysql_stmt_close(m_statement_handle)) {
            SPDLOG_ERROR("MySQLPreparedStatement: Failed to delete statement - {}.", mysql_error(m_db_handle));
        }
