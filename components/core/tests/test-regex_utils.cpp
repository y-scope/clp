...
TEST_CASE("regex_to_wildcard", "[regex_utils][regex_to_wildcard]") {
    // Test empty string
    REQUIRE(regex_to_wildcard("").value() == "");
    ...
    REQUIRE(regex_to_wildcard("\\^\\$\\.\\*\\{\\}\\[\\]\\(\\)\\+\\|\\?\\<\\>\\-\\_\\/\\=\\!\\\\")
                    .value()
            == "^$.\\*{}[]()+|\\?<>-_/=!\\\\");
    REQUIRE(regex_to_wildcard("abc\\Qdefghi\\Ejkl").error()
    ...
    REQUIRE(regex_to_wildcard("abc{-3}").error()
            == clp::regex_utils::ErrorCode::UnsupportedQuantifier);
    REQUIRE(regex_to_wildcard("abc{3,4}").error()
            == clp::regex_utils::ErrorCode::UnsupportedQuantifier);

    REQUIRE(regex_to_wildcard("{3}abc").error() == clp::regex_utils::ErrorCode::TokenUnquantifiable
    );
    REQUIRE(regex_to_wildcard("abc{3}{3}").error()
    ...
    REQUIRE(regex_to_wildcard("abc (. xyz .+ zyx .*){2} def").value()
            == "abc ? xyz ?* zyx *? xyz ?* zyx * def");
    REQUIRE(regex_to_wildcard("abc (.{3} xyz .+ zyx .*){2} def").value()
            == "abc ??? xyz ?* zyx *??? xyz ?* zyx * def");
    REQUIRE(regex_to_wildcard("abc (\\)){2} def").value() == "abc )) def");
    REQUIRE(regex_to_wildcard("abc (\\)\\*){2} def").value() == "abc )\\*)\\* def");
    REQUIRE(regex_to_wildcard("abc (x(\\*){3}z){2} def").value() == "abc x\\*\\*\\*zx\\*\\*\\*z def"
    );

    REQUIRE(regex_to_wildcard("abc (. xyz .+ zyx .*{2} def").error()
            == clp::regex_utils::ErrorCode::UnmatchedParenthesis);
    REQUIRE(regex_to_wildcard("abc (x(\\*{3}z){2} def").error()
            == clp::regex_utils::ErrorCode::UnmatchedParenthesis);
    REQUIRE(regex_to_wildcard("abc (x(\\*){3}z{2} def").error()
            == clp::regex_utils::ErrorCode::UnmatchedParenthesis);
    REQUIRE(regex_to_wildcard("abc x(\\*){3}z){2} def").error()
    ...
    REQUIRE(regex_to_wildcard("abc (x\\*){3}z){2} def").error()
            == clp::regex_utils::ErrorCode::UnmatchedParenthesis);
    REQUIRE(regex_to_wildcard("abc (abc | def){2} def").error() == clp::regex_utils::ErrorCode::Pipe
    );
    REQUIRE(regex_to_wildcard("abc (* xyz .+ zyx .*){2} def").error()
            == clp::regex_utils::ErrorCode::Star);
    REQUIRE(regex_to_wildcard("abc (+ xyz .+ zyx .*){2} def").error()
            == clp::regex_utils::ErrorCode::Plus);
    REQUIRE(regex_to_wildcard("abc (.{3}{3} xyz .+ zyx .*){2} def").error()
            == clp::regex_utils::ErrorCode::TokenUnquantifiable);
    REQUIRE(regex_to_wildcard("abc (. xyz .+{3} zyx .*){2} def").error()
            == clp::regex_utils::ErrorCode::TokenUnquantifiable);
    ...
    REQUIRE(regex_to_wildcard("x[aA").error()
            == clp::regex_utils::ErrorCode::IncompleteCharsetStructure);
    REQUIRE(regex_to_wildcard("x[]{2}z").error() == clp::regex_utils::ErrorCode::UnsupportedCharsets
    );
    REQUIRE(regex_to_wildcard("x[^]{2}z").error()
    ...
    REQUIRE(regex_to_wildcard("[Aa]").error() == clp::regex_utils::ErrorCode::UnsupportedCharsets);
    REQUIRE(regex_to_wildcard("[Ee][Xx][Cc][Ee][Pp][Tt][Ii][Oo][Nn]").error()
            == clp::regex_utils::ErrorCode::UnsupportedCharsets);
    REQUIRE(regex_to_wildcard("[eE][Xx][cC][eE][pP][Tt][iI][Oo]{2}[Nn]").error()
            == clp::regex_utils::ErrorCode::UnsupportedCharsets);
}
...
TEST_CASE(
    ...
    REQUIRE(regex_to_wildcard("[Ee][Xx][Cc][Ee][Pp][Tt][Ii][Oo][Nn]", config).value() == "exception"
    );
    REQUIRE(regex_to_wildcard("[eE][Xx][cC][eE][pP][Tt][iI][Oo]{2}[Nn]", config).value()
            == "exceptioon");

    REQUIRE(regex_to_wildcard("[eE][Xx][cC][eE][pP][Tk][iI][Oo][Nn]", config).error()
            == clp::regex_utils::ErrorCode::UnsupportedCharsets);
...
TEST_CASE("regex_to_wildcard_anchor_config", "[regex_utils][regex_to_wildcard][anchor_config]") {
    ...
    REQUIRE(regex_to_wildcard("xyz(^. xyz .* zyx .)zyx", config).error()
...
TEST_CASE("regex_trim_line_anchors", "[regex_utils][regex_trim_line_anchors]") {
    REQUIRE(regex_trim_line_anchors("") == "");
