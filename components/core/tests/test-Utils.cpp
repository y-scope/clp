...
TEST_CASE("get_bounds_of_next_potential_var", "[get_bounds_of_next_potential_var]") {
    Lexer forward_lexer;
    {
        ...
        forward_lexer.symbol_id[Lexer::TOKEN_END] = forward_lexer.symbol_id.size();
        forward_lexer.symbol_id[Lexer::TOKEN_UNCAUGHT_STRING] = forward_lexer.symbol_id.size();
        forward_lexer.id_symbol[Lexer::TOKEN_END_ID] = Lexer::TOKEN_END;
        forward_lexer.id_symbol[Lexer::TOKEN_UNCAUGHT_STRING_ID] = Lexer::TOKEN_UNCAUGHT_STRING;
    ...
    REQUIRE(get_bounds_of_next_potential_var(str, begin_pos, end_pos, is_var, forward_lexer, reverse_lexer) == true);
    REQUIRE("95" == str.substr(begin_pos, end_pos - begin_pos));
    REQUIRE(true == is_var);
