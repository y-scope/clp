...
void MySQLParamBindings::bind_varchar (size_t field_index, const char* value, size_t value_length) {
    ...
    // NOTE: binding.buffer is used for both input and output, so it is not defined as const. However, MySQL shouldn't modify it when used as an input.
    binding.buffer = const_cast<void*>(reinterpret_cast<const void*>(value));
